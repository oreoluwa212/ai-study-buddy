# backend/app.py - Updated with tier system integration
import os
import logging
...
class TierManager:
    """Handle tier checking and premium access - FIXED VERSION"""
    ...
    def can_access_flashcard_set(user_email, set_id):
        """Check if user can access a specific flashcard set"""
        if USE_SUPABASE and supabase:
            try:
                # Get the flashcard set to check its tier requirement
                result = (
                ...
                # If it's a free tier content, allow access
                if tier_required == "free":
                    return True

                # If it requires premium, check user's tier
                ...
                return False
        return True  # Default to allow access for in-memory storage

    @staticmethod
    def create_premium_subscription(user_email, plan_type, payment_id):
        """Create premium subscription after successful payment"""
        if USE_SUPABASE and supabase:
            try:
...
class PaymentStorage:
    """Updated Payment operations with better ID handling"""

    @staticmethod
    def create_payment_intent(
        amount, currency, description, user_email=None, plan_type="monthly"
    ):
        # Create payment intent without manual ID assignment for Supabase
        payment_intent = {
            "amount": amount,
            "currency": currency,
            ...
            "reference": f"studypal_{int(datetime.now().timestamp())}_{secrets.token_urlsafe(8)}",
        }
        ...
        # Fallback to memory storage with manual counter
    ...
    def update_payment_status(payment_id, status, intasend_ref=None):
        ...
        if USE_SUPABASE and supabase:
            try:
                ...
                # If payment completed, create premium subscription
                if status == "completed":
        ...
        if int(payment_id) in payment_storage:
            ...
            # Handle premium subscription for memory storage
            if status == "completed":
...
class FlashcardStorage:
    """Updated Flashcard storage operations with tier support"""
    ...
    def get_all_sets(user_email=None, include_locked=False):
        """Get flashcard sets with tier filtering"""
        if USE_SUPABASE and supabase:
            try:
                # Get all sets with tier information
                result = (
                    supabase.table("flashcard_sets")
                    .select(
                ...
                if result.data:
                    ...
                    # Add access information to each set
                    for set_data in sets:
                        tier_required = set_data.get("tier_required", "free")

                        if user_email:
                            # Check if user can access this specific set
                    ...
                    # Filter based on include_locked parameter
            ...
            [
                {
                    ...
                    "can_access": True,  # All memory sets are accessible
                    "is_locked": False,
                }
    ...
    def get_set(set_id, user_email=None):
        """Get flashcard set with access checking"""
        if USE_SUPABASE and supabase:
            try:
                # First check if user can access this set
    ...
    def delete_set(set_id, user_email=None):
        """Delete flashcard set with ownership checking"""
        if USE_SUPABASE and supabase:
            try:
...
def save_flashcards():
    """Save flashcards with tier support"""
    ...
    tier_required = data.get("tier_required", "free")  # Default to free

    if not title or not flashcards:
        return jsonify({"error": "Title and flashcards are required"}), 400

    # Check if user is trying to create premium content
...
def get_flashcard_set(set_id):
    """Get flashcard set with access control"""
    user_email = request.args.get("user_email")
...
def create_payment_intent():
    """Create a payment intent for IntaSend"""
    data = request.get_json()
    if not data:
        return jsonify({"error": "No data provided"}), 400
    ...
    plan_type = data.get("plan_type", "monthly")  # Add plan type
...
def validate_premium_access():
    """Validate premium access after payment"""
    data = request.get_json()
    ...
    return jsonify(
        {
            ...
            "tier_system": True,  # New feature flag
            "endpoints": {
...
def generate_flashcards():
    ...
    # Premium users can generate more cards
    user_tier = TierManager.check_user_tier(user_email) if user_email else "free"
